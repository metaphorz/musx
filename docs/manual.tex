...
places. \textbf{Insert abstraction} creates such a box from a path; \textbf{Save as abstraction}
downloads the current subpatch as a reusable file; and \textbf{Reload abstractions} re-fetches
...
top-right corner. \textbf{UCI Arts --- Mono MIDI Synth} is a faithful replica of Christopher
Dobrian's Max Cookbook patch \emph{Very Simple Monophonic MIDI Synthesizer}: the \texttt{midifile}
object in \emph{mono} mode with \emph{retrig} on stands in for Max's \texttt{poly 1 1} (mono,
note-stealing), its frequency drives a sawtooth \texttt{osc\~{}} (\texttt{mtof}), and the note-on
velocity scales the amplitude through an \texttt{adsr\~{}} with \emph{vel$\rightarrow$amp} enabled
(velocity $1\ldots127$ mapped to $-60\ldots0$\,dB, then \texttt{dbtoa}). It is credited to
\copyright~2017 Christopher Dobrian (UCI Arts). The \texttt{midifile} object's \texttt{track}
(isolate one part), \texttt{start} (skip seconds), \texttt{mode} (poly/mono), and \texttt{retrig}
controls make all of this configurable live. The remaining demos are \textbf{XY Synth}
